<?php
     ...
     * 专业跳转三十年
     **/
    function redirect($msg='', $url='', $t=1){
        echo '<div style="width:280px;height:130px;border:1px solid #FFC0C7;background:#FFE1EA;border-radius:8px 8px 8px 8px;margin:180px auto;">';
        echo '<div style="margin:45px auto;width:150px;">'.$msg.'</div>';
	...
	*/
	...
	/*
	专业执行增，删，改的函数（不需要结parse_str  http_build_query果集，只需要执行结果的）
	*/
	function execute($sql){
		...
		if($result && mysql_affected_rows()>0){
			...
			if(mysql_insert_id())
				return mysql_insert_id();		// 添加的时候，会返回这里
    ...
    function upload($field='file', $dir='./uploads/', $type=array('image','text'), $size=10485760){
        ...
        if($_FILES[$field]['error'] > 0){
            switch($_FILES[$field]['error']){
                ...
                case 6:
                    echo '找不到临时目parse_str  http_build_query录';
                    break;
        ...
        if(!in_array($maintype, $alow_type)){
            echo '文件类型不合法!';
            return false;
        }
	...
	function page($total,$num){
        ...
        $page = parse_str($_SERVER['QUERY_STRING'],$pap);
        ...
        $link1 = $url.http_build_query($prev);
        $link2 = $url.http_build_query($next);
